...
/*
 * Color sources: extracted from brand SVG logos + site visual language
 *
 * Rocky Mountain logo: #fcb53f (gold), #102b45 (navy), #dce4ec (light), #8b98a2 (grey)
 *   Site: bold serif headlines, dark dramatic hero, American steel heritage
 *
 * Interpro logo: #f27f20 (orange), #123549 (teal-navy), #dce4ec (light), #8b98a2 (grey)
 *   Site: similar dark industrial, Canadian energy, slightly warmer navy
 */

const themes = {
  rocky: {
    name: "Rocky Mountain",
    // From logo: #fcb53f gold accent
    accent: "#fcb53f",
    accentDark: "#d9982e",
    accentDim: "#fcb53f12",
    accentMid: "#fcb53f25",
    // Derived from logo navy #102b45
    bg: "#070e17",
    bgCard: "#0d1926",
    bgInput: "#0a1420",
    border: "#1a3050",
    borderLight: "#1a305025",
    // From logo: #dce4ec, #8b98a2
    textPrimary: "#dce4ec",
  ...
  interpro: {
    name: "Interpro",
    // From logo: #f27f20 orange accent
    accent: "#f27f20",
    accentDark: "#d06a18",
    accentDim: "#f27f2012",
    accentMid: "#f27f2025",
    // Derived from logo teal-navy #123549
    bg: "#081620",
    bgCard: "#0f2231",
    bgInput: "#0a1a28",
    border: "#1a3a4e",
...
const RockyLogo = ({ h = 34, textColor = "#dce4ec", subtitleColor = "#8b98a2" }) => (
...
export default function EVRlockLayout() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.bg, fontFamily: t.fontBody, position: "relative", transition: tr }}>
      {/* Google Fonts */}
      <link href="https://fonts.googleapis.com/css2?family=Libre+Baskerville:wght@400;700&family=Source+Sans+3:wght@400;600&family=DM+Serif+Display&family=DM+Sans:wght@400;500&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet" />

      {/* Decorative lines */}
      <div style={{ position: "fixed", left: 40, top: 0, bottom: 0, width: 120, opacity: 0.04, pointerEvents: "none", zIndex: 0 }}>
        {[0, 20, 44, 60, 85, 105].map((x, i) => (
          <div key={i} style={{ position: "absolute", left: x, top: 0, bottom: 0, width: 1, background: `linear-gradient(180deg, transparent 0%, ${t.textSecondary} ${30 + i * 5}%, ${t.textSecondary} ${60 - i * 3}%, transparent 100%)`, transition: tr }} />
        ))}
      </div>

      {/* ─── HEADER ─── */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 40px", maxWidth: 900, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div style={{ cursor: "pointer" }}>
          {skin === "rocky" ? <RockyLogo /> : <InterproLogo />}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {/* Skin swap */}
          <div
            onClick={() => setSkin(skin === "rocky" ? "interpro" : "rocky")}
            style={{ fontFamily: t.fontMono, fontSize: 9, color: t.textMuted, cursor: "pointer", letterSpacing: 0.5, padding: "6px 12px", borderRadius: 8, border: `1px solid ${t.border}`, transition: tr, display: "flex", alignItems: "center", gap: 6, userSelect: "none" }}
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={t.textMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>
            {skin === "rocky" ? "INTERPRO" : "ROCKY MTN"}
          </div>
          <span style={{ fontFamily: t.fontMono, fontSize: 11, color: t.navText, cursor: "pointer", letterSpacing: 0.5 }}>CONTACT</span>
          <div onClick={() => setShelfOpen(!shelfOpen)} style={{ width: 38, height: 38, borderRadius: 10, background: shelfOpen ? t.accent : t.bgCard, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: tr, border: `1px solid ${shelfOpen ? t.accent : t.border}` }} title="Resources">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={shelfOpen ? t.bg : t.textSecondary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* ─── SHELF ─── */}
      <div onClick={() => setShelfOpen(false)} style={{ position: "fixed", inset: 0, background: "#0009", zIndex: 90, opacity: shelfOpen ? 1 : 0, pointerEvents: shelfOpen ? "auto" : "none", transition: "opacity 0.3s ease" }} />
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: 340, background: t.bgCard, borderLeft: `1px solid ${t.border}`, zIndex: 100, transform: shelfOpen ? "translateX(0)" : "translateX(100%)", transition: "transform 0.35s cubic-bezier(0.22,1,0.36,1), background 0.5s ease", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        <div style={{ padding: "20px 20px 12px", borderBottom: `1px solid ${t.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontFamily: t.fontMono, fontSize: 10, color: t.accent, letterSpacing: 2, marginBottom: 2 }}>RESOURCES</div>
            <div style={{ fontSize: 14, color: t.textMuted }}>All connection documents</div>
          </div>
          <div onClick={() => setShelfOpen(false)} style={{ width: 30, height: 30, borderRadius: 8, background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={t.textMuted} strokeWidth="2" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </div>
        </div>
        {shelfData.map((g) => (
          <div key={g.connection} style={{ padding: "16px 20px", borderBottom: `1px solid ${t.borderLight}` }}>
            <div style={{ fontFamily: t.fontMono, fontSize: 10, color: t.accent, letterSpacing: 1, marginBottom: 10 }}>{g.connection.toUpperCase()}</div>
            {g.docs.map((doc, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < g.docs.length - 1 ? `1px solid ${t.borderLight}` : "none", cursor: "pointer" }}>
                ...
                <span style={{ fontSize: 13, color: t.textSecondary, lineHeight: 1.3 }}>{doc}</span>
      ...
      {/* ─── MAIN COLUMN ─── */}
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "20px 40px 60px", position: "relative", zIndex: 1 }}>

        {/* User message */}
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
            <div style={{ background: t.bgCard, borderRadius: 12, padding: "10px 16px", maxWidth: 400, transition: tr }}>
              <span style={{ fontSize: 14, color: t.textSecondary }}>tell me about QB2 for 7" thermal wells</span>
            </div>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: t.bgCard, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 32 }}>
          <div style={{ width: 28, height: 28, borderRadius: 6, background: `linear-gradient(135deg, ${t.accent}, ${t.accentDark})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2, transition: tr }}>
            <span style={{ fontSize: 12, color: "#fff", fontWeight: 600, fontFamily: t.fontDisplay }}>E</span>
          </div>
          <div style={{ flex: 1 }}>
            {[
              ...
              <p key={i} style={{ fontSize: 15, color: t.textSecondary, lineHeight: 1.75, margin: "0 0 16px 0", fontFamily: t.fontBody, transition: tr }}>{p}</p>
            ))}
            <div style={{ borderTop: `1px solid ${t.border}`, paddingTop: 16, marginTop: 24, transition: tr }}>
              <div style={{ fontFamily: t.fontMono, fontSize: 10, color: t.textMuted, letterSpacing: 2, marginBottom: 10, textTransform: "uppercase" }}>References</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {["[1] QB2 Supplementary Data →", "[2] QB2 Running Procedures →", "[3] QB2 Field Bulletin →"].map((r) => (
                  <span key={r} style={{ fontFamily: t.fontMono, fontSize: 11, color: t.textSecondary, background: t.bgCard, borderRadius: 20, padding: "6px 14px", cursor: "pointer", transition: tr }}>{r}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {/* Suggested prompts */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20, paddingLeft: 40 }}>
          {["Compare QB2 vs QB1-HT for this size →", "Show me all 7\" P110 options →", "What about QB2-XL for larger diameters? →"].map((p) => (
            <span key={p} style={{ fontFamily: t.fontMono, fontSize: 11, color: t.textSecondary, border: `1px solid ${t.border}`, borderRadius: 20, padding: "7px 16px", cursor: "pointer", transition: tr }}>{p}</span>
          ))}
        </div>

        {/* Input */}
        <div style={{ background: t.bgCard, borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 48, marginLeft: 40, border: `1px solid ${t.border}`, transition: tr }}>
          ...
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: `linear-gradient(135deg, ${t.accent}, ${t.accentDark})`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, transition: tr }}>
            <span style={{ color: "#fff", fontSize: 14, transform: "rotate(45deg)", display: "block" }}>➤</span>
          </div>
        </div>

        {/* ─── PERFORMANCE DATA ─── */}
        <div style={{ paddingLeft: 40 }}>
          <div style={{ fontFamily: t.fontMono, fontSize: 10, color: t.textMuted, letterSpacing: 2, marginBottom: 16, textTransform: "uppercase" }}>Referenced Performance Data</div>

          <div style={{ background: t.bgCard, borderRadius: 12, border: `1px solid ${t.border}`, overflow: "hidden", marginBottom: 20, transition: tr }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${t.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div>
                <span style={{ fontFamily: t.fontMono, fontSize: 10, color: t.accent, letterSpacing: 1, marginRight: 12 }}>1</span>
                <span style={{ fontSize: 15, color: t.textPrimary, fontWeight: 500, fontFamily: t.fontDisplay }}>QB2 Premium — 7" P110</span>
              </div>
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                {["USC", "METRIC"].map((u, i) => (
                  <span key={u} style={{ fontFamily: t.fontMono, fontSize: 9, color: i === 0 ? t.textPrimary : t.textMuted, background: i === 0 ? t.border : "transparent", borderRadius: 4, padding: "3px 8px", cursor: "pointer" }}>{u}</span>
                ))}
                <div style={{ width: 1, height: 16, background: t.border, margin: "0 4px" }} />
                <span style={{ fontFamily: t.fontMono, fontSize: 9, color: t.accent, background: t.accentDim, borderRadius: 6, padding: "4px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
            ...
            {[
              ...
              <div key={si} style={{ padding: "12px 16px", borderBottom: si < 2 ? `1px solid ${t.borderLight}` : "none" }}>
                <div style={{ fontFamily: t.fontMono, fontSize: 9, color: t.textMuted, letterSpacing: 1, marginBottom: 8 }}>{section.label}</div>
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${section.cols}, 1fr)`, gap: 6 }}>
                  {section.rows.map(([k, v]) => (
                    <div key={k} style={{ padding: "6px 8px", background: t.bg, borderRadius: 6, transition: tr }}>
                      <div style={{ fontFamily: t.fontMono, fontSize: 9, color: t.textMuted, marginBottom: 2 }}>{k}</div>
                      <div style={{ fontFamily: t.fontMono, fontSize: 13, color: t.textPrimary }}>{v}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* PDF docs card */}
          <div style={{ background: t.bgCard, borderRadius: 12, border: `1px solid ${t.border}`, overflow: "hidden", transition: tr }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${t.border}`, display: "flex", alignItems: "center" }}>
              <span style={{ fontFamily: t.fontMono, fontSize: 10, color: t.accent, letterSpacing: 1, marginRight: 12 }}>2</span>
              <span style={{ fontSize: 15, color: t.textPrimary, fontWeight: 500, fontFamily: t.fontDisplay }}>QB2 Technical Documents</span>
            </div>
            <div style={{ padding: "8px 16px" }}>
              {[
                ...
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: i < 4 ? `1px solid ${t.borderLight}` : "none", cursor: "pointer" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 6, background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: t.fontMono, fontSize: 8, color: t.accent, transition: tr }}>PDF</div>
                    <span style={{ fontSize: 13, color: t.textSecondary, fontFamily: t.fontBody }}>{doc.name}</span>
                  </div>
                  <span style={{ fontFamily: t.fontMono, fontSize: 10, color: t.textMuted }}>{doc.size}</span>
                </div>
              ))}
            </div>
          </div>
